...
/**
* do classification and output the results on file
**/
    ...
    char* resultFile )
{
	...
	ofstream myfile;
  	myfile.open (resultFile);
	    ...
	    myfile<<i<<","<<predictedClassIndex<<endl;
            //if (predictedClassIndex == instanceTable[i].classIndex) correctCounter++;
        }

	myfile.close();
        printf( "Classification results written on file %s\n", resultFile );
    }
...
/**
* do classification and compute accuracy
**/
void Classifier::Classify(
    const Instance* instanceTable,
    const unsigned int numInstances )
