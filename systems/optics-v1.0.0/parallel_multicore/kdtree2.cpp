...
float kdtree2::getDistance(int idx, int idy)
{
	int i, j;
	float dis_local = 0.0;
    	for (j = 0; j < dim; j++) 
	{
      		...
      		dis_local += squared(the_data[idx][j] - the_data[idy][j]);
    	}
...
kdtree2_node* kdtree2::build_tree_for_range(int l, int u, kdtree2_node* parent) {
  ...
  if ((u-l) <= bucketsize) {
	...
	//if(1164013 == l && u == 2225537)
	//{
	//	cout << endl;
	//	for (int k=l; k <= u; k++)
	//		if(the_data[ind[k]][c] > 49)
	//		cout << the_data[ind[k]][c] << " ";
	//	cout << endl;
	//}
	...
	//cout << "build_tree_for_range avarge " << average << " sum " << sum << endl;
	
	//if(sum < 0)
	//	cout << "WHY sum is negative " << sum << endl;
	...
	// ADDITIONAL CODE
	vector <float> data_median_comp;
	data_median_comp.reserve(u-l+1);

	for (int k=l; k <= u; k++) {
      data_median_comp.push_back(the_data[ind[k]][c]);
    }
	
	float median;
	median = findKMedian(data_median_comp, data_median_comp.size()/2);
	data_median_comp.clear();
	...
	//cout << "build_tree_for_range l " << l << " u " << u << " m " << m << " dims " << c << endl;
